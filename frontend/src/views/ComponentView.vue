...
const { fetchComponents, fetchRegistryManifest } = useApi()
...
const activeTab = ref<'info' | 'ports' | 'env' | 'registry'>('info')
...
const tabs = computed(() => [
  ...
  ...(registryManifest.value ? [{ id: 'registry' as const, label: 'Сборка', icon: 'cube' as const }] : []),
])
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="flex border-b border-gray-200 dark:border-slate-700">
          <button
            ...
            :class="[
              'inline-flex items-center gap-1.5 px-4 py-3 text-sm font-medium border-b-2 transition -mb-px',
              activeTab === tab.id
